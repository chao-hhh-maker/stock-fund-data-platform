...
> **项目名称**：股票基金数据获取和管理平台  
> **课程模块**：模块 2：AI 辅助设计  
> **文档定位**：ER 图、数据表、字段含义、约束与索引设计  
> **最终报告映射**：最终报告第 3 章 3.4 节  
> **代码依据**：backend/app/models.py、sql/init.sql、backend/stock_fund_platform.db  
> **整理日期**：2026 年 6 月
...
- `models.py` 是运行时数据库结构的主要依据，应用启动时通过 SQLAlchemy 自动建表。
- `sql/init.sql` 用于 MySQL 部署和课程文档交付，字段含义与 ORM 保持一致。
- SQLite 默认运行方式保证教师或助教不安装 MySQL 也能直接启动系统。
- MySQL 迁移时需注意 `trigger` 是保留字，SQL 脚本中应加反引号。

## 提交前核对

- [ ] ER 图包含用户、角色、租户、标的、行情、净值、任务、运行、导出、审计、告警、质量问题等核心实体。
- [ ] 高频查询表包含 `(code, date)` 组合索引。
- [ ] 采集数据表设置唯一约束，支持重复采集幂等 upsert。
