...
> 选题二：股票基金数据获取和管理平台
>
> 当前阶段：完整增强版已交付（6 大模块全覆盖 + 深色金融科技风前端 + 38 项测试全过）
...
1. 补齐 README 中的负责人、团队成员、仓库地址和时间计划。
2. 依据施工文档创建任务看板，并为 5 名成员分配模块职责。
3. 先完成 user_stories.md 和 use_cases.md，再进入设计与编码阶段。
4. 从第一天开始同步维护 ai.md 和 assign.md，避免后期补文档失真。
